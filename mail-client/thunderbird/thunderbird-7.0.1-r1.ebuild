...
IUSE="bindist gconf +crashreporter +crypt +ipc +lightning mozdom +globalmenu"
...
src_prepare() {
	...
	if use globalmenu ; then
		cd ${S}
		tar xaf ${FILESDIR}/globalmenu.tar.xz -C mozilla/extensions
...
src_install() {
	...
	if use globalmenu; then
		unzip -qo -d "${D}/${MOZILLA_FIVE_HOME}/extensions/globalmenu@ubuntu.com" "${S}/mozilla/dist/xpi-stage/globalmenu.xpi"
	fi
